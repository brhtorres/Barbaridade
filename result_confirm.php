<?php 
  ...
  function createCards() {    
      ...
      echo '<div class=" col-lg-3 col-md-4 col-sm-6 col-10 ">';
      ...
      echo      '<p class="card-text">Confirmar até: '.date('d/m/Y', strtotime($row["data_confirm"])) .'</p>';
      echo            '<form method="POST" action="confirmed.php">';
      echo              '<input id="name" name="name" type="hidden" value='.$row["nome_cliente"].'>';
      echo              '<input id="phone" name="phone" type="hidden" value=' .$row["tel_cliente"].'>';
      echo              '<input id="idReserva" name="idReserva" type="hidden" value='.$row['idReserva'].'>';
      echo              '<input id="data_confirm" name="data_confirm" type="hidden" value='.$row['data_confirm'].'>';      
        ...
        <?php
           if (mysqli_num_rows($resultado) == 0) {
            echo "<div class='alert alert-info' role='alert'>
                    Você não possui confirmação a realizar. Veja suas reservas na aba Reservas.
                  </div>";    
              exit;
          }
        ?>
